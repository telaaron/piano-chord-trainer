<script lang="ts">
	...
	$effect(() => {
		...
		const unsub = onAuthChange(async (s) => {
			auth = s;
			if (s.user) {
				const { data } = await getSupabase().from('profiles').select('role').eq('id', s.user.id).single();
...
<div class="min-h-dvh flex flex-col">
	...
	<nav class="sticky top-0 z-10 border-b border-[var(--wood-dark)]/60 bg-[var(--bg)]/75 backdrop-blur-md">
		...
		<a href="/" class="flex items-center gap-2 group">
			<img src="/favicon/favicon-96x96.png" alt="Chord Trainer" class="h-8 w-8 object-contain"
		...
		<div class="flex items-center gap-1 text-sm">
			...
			<div class="hidden sm:flex items-center gap-1">
				...
				{#if isAdmin}
					<a
						href="/admin"
						class="px-3 py-1.5 rounded-sm transition-colors {page.url.pathname === '/admin' ? 'bg-[var(--primary-muted)] text-[var(--primary)] font-medium' : 'text-[var(--text-dim)] hover:text-[var(--text)]'}"
			...
			{#if !auth.loading}
				{#if auth.user}
					<a
						href="/account"
						class="ml-1 px-3 py-1.5 rounded-sm transition-colors text-sm {page.url.pathname === '/account' ? 'bg-[var(--primary-muted)] text-[var(--primary)] font-medium' : 'text-[var(--text-muted)] hover:text-[var(--text)]'}"
					>
						{t('nav_auth.account')}
					</a>
				{:else}
		...
		<!-- Mobile dropdown -->
		{#if mobileMenuOpen}
			<div class="sm:hidden absolute top-full left-0 right-0 z-20 border-t border-[var(--border)] bg-[var(--bg)]/95 backdrop-blur-md px-4 py-3 flex flex-col gap-1 shadow-lg">
				<a href="/learn"
					class="px-3 py-2 rounded-sm transition-colors text-sm {page.url.pathname.startsWith('/learn') ? 'bg-[var(--primary-muted)] text-[var(--primary)] font-medium' : 'text-[var(--text-muted)]'}">
					{t('nav.learn')}
				</a>
				<a href="/train"
					class="px-3 py-2 rounded-sm transition-colors text-sm {page.url.pathname.startsWith('/train') ? 'bg-[var(--primary-muted)] text-[var(--primary)] font-medium' : 'text-[var(--text-muted)]'}">
					{t('nav.train')}
				</a>
				<a href="/for-educators"
					class="px-3 py-2 rounded-sm transition-colors text-sm {page.url.pathname === '/for-educators' ? 'bg-[var(--primary-muted)] text-[var(--primary)] font-medium' : 'text-[var(--text-muted)]'}">
					{t('nav.educators')}
				</a>
				<a href="/pricing"
					class="px-3 py-2 rounded-sm transition-colors text-sm {page.url.pathname === '/pricing' ? 'bg-[var(--primary-muted)] text-[var(--primary)] font-medium' : 'text-[var(--text-muted)]'}">
					{t('nav_auth.pricing')}
				</a>
				{#if isAdmin}
					<a href="/admin"
						class="px-3 py-2 rounded-sm transition-colors text-sm {page.url.pathname === '/admin' ? 'bg-[var(--primary-muted)] text-[var(--primary)] font-medium' : 'text-[var(--text-dim)]'}">
						⚙ Admin
					</a>
				{/if}
			</div>
		...
		<div class="bg-[var(--primary)]/10 border-b border-[var(--primary)]/20 px-4 py-2.5">
			<div class="max-w-[1600px] mx-auto flex items-center justify-between gap-4 text-sm">
				...
				<div class="flex items-center gap-2 shrink-0">
					<a href="/auth/login" class="px-3 py-1 rounded-sm bg-[var(--primary)] text-white text-xs font-medium hover:bg-[var(--primary-hover)] transition-colors">
						{t('nav_auth.sync_banner_cta')}
					</a>
					<button onclick={dismissSyncBanner} class="text-xs text-[var(--text-dim)] hover:text-[var(--text-muted)] transition-colors">
		...
		<footer class="px-4 sm:px-6 py-8 border-t border-[var(--wood-dark)]/40">
			<div class="max-w-5xl mx-auto space-y-5">
				...
				<div class="flex flex-wrap gap-x-5 gap-y-2 text-xs text-[var(--text-dim)]">
					...
					<a href="/pricing" class="hover:text-[var(--text-muted)] transition-colors">{t('nav_auth.pricing')}</a>
					...
					<a href="/terms" class="hover:text-[var(--text-muted)] transition-colors">{t('nav_auth.terms')}</a>
